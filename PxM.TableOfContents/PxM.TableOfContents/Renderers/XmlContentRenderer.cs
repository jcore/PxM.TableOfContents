...
namespace PxM.TableOfContents.Renderers
{
    ...
    public class XmlContentRenderer : InDesignItemRendererBase
    {
        ...
        {
            return Constants.Fields.StaticTextFieldName;
        }
        ...
        protected override void RenderContent(PrintContext printContext, [NotNull] XElement output)
        {
            ...
            try
            {
                ...
                tempContentNode = XElement.Parse(string.Format("<TempContentRoot>{0}</TempContentRoot>", content.Contains("CDATA") ? content.RemoveBadHtml() : content.Replace("&", "&amp;").RemoveBadHtml()));
            }
            catch (Exception exc)
            {
            ...
            if (!tempContentNode.HasElements)
            {
                ...
                foreach (var plainTextNode in plainTextNodes)
                {
                    ...
                    plainTextNode.ReplaceWith(new XCData(content.Contains("CDATA") ? plainTextNode.ToString() : plainTextNode.ToString().Replace("&amp;", "&").Replace("&amp;", "&").RemoveBadHtml()));
                }

                HtmlExtensions.RemoveEmptyParagraphStyles(tempContentNode);
        ...
        protected virtual string ParseContent(PrintContext printContext)
        {
            ...
            if (field is DateField)
            {
                if (!string.IsNullOrEmpty(field.Value))
                {
                    ...
                    {
                        return AddWrapping(((DateField)field).DateTime.ToString(HttpUtility.UrlDecode(DateFormat)));
                    }
                    else
                    {
            ...
            else if (field is HtmlField)
            {
                ...
                {
                    return AddWrapping(RichTextParser.ConvertToXml(fieldValue.StripHtmlTags(), context, printContext.Language));
                }
                try
                {
                    return AddWrapping(RichTextParser.ConvertToXml(fieldValue.RemoveBadHtml(), context, printContext.Language));
                }
                catch (Exception ex)
                {
        ...
        public  ParseDefinitionCollection GetParseDefinitionCollection(Item printItem)
        {
            ...
            if (!string.IsNullOrEmpty(printItem[Constants.Fields.TransformationSet]) && ID.IsID(printItem[Constants.Fields.TransformationSet]))
                setItem = database.GetItem(printItem[Constants.Fields.TransformationSet]);
            ...
            {
                if (!string.IsNullOrEmpty(t[Constants.Fields.TransformationSet]))
                    return ID.IsID(t[Constants.Fields.TransformationSet]);
                else
                    return false;
            }));
            if (obj2 != null)
                setItem = database.GetItem(obj2[Constants.Fields.TransformationSet]);
            if (setItem != null)
